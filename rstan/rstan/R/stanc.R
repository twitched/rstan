...
stanc_beta <- function(model_code, model_name, isystem) {
  ...
  stanc_call <- paste0("stanc('", model_name, "', '", 
                       paste(model_code, collapse = "\n"), "')")
  model_cppcode <- try(ctx$eval(stanc_call), silent = TRUE)
  model_cppcode <- paste0("#include <boost/integer/integer_log2.hpp>", model_cppcode)
  if (!is.character(model_cppcode) || 
      grepl("Code generated by stanc", model_cppcode, fixed = TRUE)) {
